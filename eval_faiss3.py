"""
Evaluate the FAISS v3 RAG system against the ex2.json test set.

Answers are produced by faiss3.answer_question_with_sources() — the exact same
pipeline users experience (DoclingLoader chunking, hybrid retrieval, LLM-as-judge).
...
from faiss3 import answer_question_with_sources, create_or_load_faiss_index, NEBIUS_API_KEY, NEBIUS_BASE_URL, LLM_MODEL
...
def main(label: str = RUN_LABEL):
    ...
    # Initialise faiss3's global state (index + retriever)
    ...
    for i, case in enumerate(test_cases):
        ...
        judge_result = judge(nebius_client, question, reference, generated_text)
        abstain = detect_abstention(generated_text)

        print(f"  → {judge_result.get('correctness','?')} | hallucination={judge_result.get('hallucination')} | citation={judge_result.get('has_citation')} | sources={len(sources)}")
    ...
    results = {
        "model_name": f"faiss3-rag ({LLM_MODEL})",
        "num_questions": len(test_cases),
        "aggregates": agg,
        "per_question": per_question,
    }

    out_path = "evaluation_results_faiss3-rag.json"
